...
fn main() {
    ...
    let window = WindowBuilder::new()
        ...
        .build(&event_loop)
        .expect("Failed to create window in main");
    let mut configure_swapchain = true;

    let instance = backend::Instance::create(name, 1).expect("GFX backend not supported");
    let surface = unsafe {
        instance
            .create_surface(&window)
            .expect("Failed to create surface in main")
    };
    ...
    let (device, mut queue_group) = {
        ...
        let queue_family = adapter
            ...
            .expect("Failed to find compatible queue family in main");
...
unsafe fn push_constant_bytes<T>(push_constants: &T) -> &[u32] {
    ...
    std::slice::from_raw_parts(start_ptr, size_u32)
}
